...
function setup() {
  ...
  //move canvas into the parent in the HTML file
...
function draw() {
  ...
  while (count<numBubbles){
    ...
    if (attempts > 500||bubbles.length>maxBubs){
      //exit condition if it can't find a place for another bubbles, however many times
      print("No more spots");
      noLoop();
      done = true; //now allow mouse click reset.

      //display the message "Click mouse to reset"
...
function mouseClicked(){
  if (done){
    //reset the bubbles, perlin noise, and colors. Only if the bubbles are done to prevent overflow problems
    bubbles = [];
    perNoise = random(100);
    scaleCols();
    loop();

    //hide the click the mouse to reset message
    var msg = document.getElementById("resetMsg");
    msg.style.display = "none";

    //no longer done sine it is restarting
    done = false;
  }
}
