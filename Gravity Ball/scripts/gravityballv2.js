...
V2
Updates: Color no longer changes rapidly while rollling, and random gravity*/
...
//This is the object, ball. It has the methods move, display, and setup. The set-up is the constructor
    ...
    if (this.y+this.radius > h && this.yspeed > 0) {
     ...
     if (this.yspeed < -1){
     colorChange(); //each bounce, change color of the Ball, UNLESS the ball is done bouncing
    }
...
//show the ball
  ...
  this.setup = function(){
    ...
    this.a = random(.005, 0.06); //Acceleration, similar to the g constant in physics, set to a random small value
    }
}
